...
import { DoctorRepository, PatientRepository } from '@db/repositories';
...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.user.role === UserRole.DOCTOR) {
      const doctor = await this.doctorRepository.findById(request.user._id);
      ...
      const patient = await this.patientRepository.findById(request.user._id);
